...
class GameTime:
    hour: int = 0
    day: int = 1
    time_scale: float = 60.0  # 1 game hour = 1 real second at 60 FPS
...
class TimeSystem:
    ...
    def update(self, delta_time: float):
        ...
        # Process accumulated time into hours
        while self.accumulated_time >= 1.0:
    ...
    def render(self, screen: pygame.Surface):
        """Render current time"""
        time_str = f"Day {self.time.day} - {self.time.hour:02d}:00 ({self.time.time_of_day})"
