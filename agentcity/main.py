...
class AgentCity(Game):
    ...
    def _handle_mouse_click(self, event):
        ...
        if self.city.agents:
            nearest_agent = min(
                self.city.agents,
                key=lambda a: ((a.state.position[0] - pos[0])**2 + 
    ...
    def update(self):
        ...
        # Update city and agents
        self.city.update(delta_time * self.time_scale, self.time_system.time.time_of_day)

    def render(self):
        # Clear screen with sky color
        sky_color = (150, 200, 255) if not self.time_system.time.is_night else (20, 20, 50)
